...
def add_to_bag(request, item_id):
    """ Add a product with size, quantity and rental dates to the booking cart """
    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]['items_by_size'].keys():
                # Update quantity and dates for existing size
                bag[item_id]['items_by_size'][size]['quantity'] += quantity
                bag[item_id]['items_by_size'][size]['start_date'] = start_date
                bag[item_id]['items_by_size'][size]['end_date'] = end_date
                messages.success(
                    request,
        ...
        # No size product (shouldn't happen but handle gracefully)
        if item_id in list(bag.keys()):
